...
!f2py intent(in,out,overwrite) HEIGHT,SLIG
            ...
            !	  MAXLIG: level of irradiance at midday
                  ...
                  pi2=8.*atan(1.)          !        2 times pi
                  deg=360./pi2             !        convert from radians to degrees
                  rad=pi2/360.   ! Degrees to radians

                  P = 3.1415927
                  TWLIGHT = 5.76
                            
                    DELTA = .3979*SIN(rad*.9856*(D-80)+ &
                    1.9171*(SIN(rad*.9856*D)-.98112))

                    H12 = DELTA*SIN(rad*B*1.)-&
                    SQRT(1.-DELTA**2)*COS(rad*B*1.)*COS(rad*15.*12)
            
                    HEIGHT = DELTA*SIN(rad*B*1.)-&
                    SQRT(1.-DELTA**2)*COS(rad*B*1.)*COS(rad*15.*H)
